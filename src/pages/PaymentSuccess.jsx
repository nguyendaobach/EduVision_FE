...
const PaymentSuccess = () => {
  ...
  useEffect(() => {
    ...
    const checkPayment = async () => {
      try {
        console.log("PaymentSuccess: Getting payment status for orderCode:", orderCode);
        ...
        const paymentStatus = result?.status || result?.payload?.status || status;
        
        setPaymentDetails({
          orderCode,
          status: paymentStatus,
          amount: result?.payload?.amount || result?.amount,
          createdAt: result?.payload?.createdAt || result?.createdAt || new Date().toISOString()
        });
        ...
        if (paymentStatus === "pending" || !paymentStatus || 
          ...
          console.log("PaymentSuccess: Status is pending, triggering expired payment check");
          try {
            ...
            const updatedStatus = updatedResult?.status || updatedResult?.payload?.status;
            ...
            console.error("PaymentSuccess: Error checking expired payments:", expiredCheckError);
          }
        }

        // Notify Dashboard về payment được cập nhật
        localStorage.setItem('lastUpdatedPayment', JSON.stringify({
          orderCode,
          status: paymentStatus,
          timestamp: Date.now()
        }));
        ...
        if (error.response?.status === 500) {
          notify.error("Lỗi server khi kiểm tra trạng thái thanh toán. Vui lòng thử lại sau.");
        } else if (error.response?.status === 404) {
          notify.error("Không tìm thấy giao dịch với mã này.");
        } else {
          notify.error("Không kiểm tra được trạng thái thanh toán: " + (error.message || "Lỗi không xác định"));
        }
    ...
    checkPayment();
  }, [searchParams, dispatch, notify, navigate]);
  ...
  const isSuccess = paymentDetails?.status === "success" || paymentDetails?.status === "SUCCESS" || paymentDetails?.status === "PAID";
  const isFailed = paymentDetails?.status === "failed" || paymentDetails?.status === "FAILED";
  const isCancelled = paymentDetails?.status === "cancelled" || paymentDetails?.status === "CANCELLED";
      ...
      <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full text-center">
        {isSuccess ? (
          <>
            ...
            <p className="text-gray-600 mb-6">Quota đã được nạp vào tài khoản của bạn</p>
          </>
        ) : isFailed ? (
          <>
          ...
          <>
            ...
            <h1 className="text-2xl font-bold text-gray-800 mb-2">Thanh toán đã hủy</h1>
            <p className="text-gray-600 mb-6">Giao dịch đã bị hủy do hết thời gian chờ hoặc người dùng hủy.</p>
          </>
        ) : (
          <>
            ...
            <p className="text-gray-600 mb-6">Giao dịch đang được xử lý. Vui lòng chờ trong giây lát.</p>
          </>
        )}

        {paymentDetails && (
          <div className="bg-gray-50 rounded-lg p-4 mb-6 text-left">
            <h3 className="font-semibold text-gray-800 mb-2">Chi tiết giao dịch:</h3>
            <div className="space-y-1 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500">Mã giao dịch:</span>
                <span className="text-gray-800">{paymentDetails.orderCode}</span>
              </div>
              {paymentDetails.amount && (
                <div className="flex justify-between">
                  <span className="text-gray-500">Số tiền:</span>
                  <span className="text-gray-800">{paymentDetails.amount.toLocaleString()} VNĐ</span>
                </div>
              )}
              <div className="flex justify-between">
